...
module.exports = {
	...
	async run(client, int, player, embed) {
		...
		await int.reply({
			embeds: [embed(int.guild, int.member.user).setTitle(`Ses ${value} olarak ayarlandı!`).setThumbnail("https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTG5mKjOY5FPlDgrffJo-tiuxtQVazWol8ipA&usqp=CAU")],	ephemeral: true
		});
	}
};